...
def parse_args():
    ...
    parser.add_argument("--model_name_or_path", type=str, default='mistral-1-rag-baseline')  # model path
    parser.add_argument("--dataset", type=str, default='../reward/test_data.json')  # data path
    parser.add_argument("--tensor_parallel_size", type=int, default=1)  # tensor_parallel_size
    parser.add_argument("--output_dir", type=str, default="data/mistral1_afterrlhf_o3_baseline.json")  # output location
...
if __name__ == "__main__":
    ...
    sampling_params = SamplingParams(n=1, temperature=0.0, top_p=1, max_tokens=2048, stop=[], seed=args.random_seed)
    print('sampling =====', sampling_params)
    llm = LLM(model=args.model_name_or_path,tensor_parallel_size=args.tensor_parallel_size, dtype = "float16",enforce_eager=True, gpu_memory_utilization=0.95,swap_space=32,trust_remote_code=True)
