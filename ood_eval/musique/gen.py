...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="data/llama31_musique_dev_n4_reason_seed2.json")  # output location
    parser.add_argument("--random_seed", type=int, default=2)  # random seed
...
if __name__ == "__main__":
    ...
    for sample in dataset:
        string = ""
        paragraph = sample['paragraphs']
        for s in paragraph:
            string += s['title'] + "\n\n"
            string += s['paragraph_text'] + "\n\n"
        
        prompt = QA_prompt.format(question=sample['question'],passages=string)
        gen_dataset.append({"prompt":prompt,"answer":[sample['answer']] + sample['answer_aliases'],"reference":string})
    
    sampling_params = SamplingParams(n=1, temperature=1.0, top_p=1, max_tokens=4096, stop=[], seed=args.random_seed)
    print('sampling =====', sampling_params)
    llm = LLM(model=args.model_name_or_path,tensor_parallel_size=args.tensor_parallel_size, dtype = "float16",enforce_eager=True, gpu_memory_utilization=0.8,swap_space=128,trust_remote_code=True)
